...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  private whiteListRoutes: string[] = [
    '/login',
    '/register',
    '/forgot-password',
    '/coming-soon',
  ...
  constructor(
    ...
    effect(async () => {
      if (environment.production) {
        ...
        var acceptedLinks = user.functionRoles.map((x: any) => x.functionLink) || [];
        if (
          acceptedLinks &&
          acceptedLinks.length > 0 &&
          this.whiteListRoutes.indexOf(this.router.url) < 0 &&
          acceptedLinks.indexOf(this.router.url) < 0
        ) {
  ...
  updateProps(config: ILayout) {
    ...
    this.appHeaderDisplay = environment.isForNx
      ? false
      : (this.layout.getProp('app.header.display', config) as boolean);
    this.appFooterDisplay = this.layout.getProp(
      'app.footer.display',
      config
    ) as boolean;
    this.appSidebarDisplay = environment.isForNx
      ? false
